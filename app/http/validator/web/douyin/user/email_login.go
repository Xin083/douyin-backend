...
func (e EmailLogin) CheckParams(ctx *gin.Context) {
	//1.基本的验证规则没有通过
	...
	extraAddBindDataContext := data_transfer.DataAddContext(e, consts.ValidatorPrefix, ctx)
	if extraAddBindDataContext == nil {
		response.ErrorSystem(ctx, "email_login 表单验证器json化失败", "")
	} else {
		// 验证完成，调用控制器，写具体业务逻辑
		(&web.UserController{}).EmailLogin(extraAddBindDataContext)
	}
}
